---
title: "PSY8712 Final Project"
author: "Merhar"
date: "`r Sys.Date()`"
output: 
  html_document:
    toc: true # to add a table of contents for easier reading
    toc_float: true # so you can access the toc even when you scroll on the doc
    df_print: paged
---
...
week8_tbl <- read_csv("../data/week3.csv") %>%
  mutate(timeEnd = ymd_hms(timeEnd),
         # timeStart = ymd_hms(timeEnd), # even though you don't need this in the final calculation, you need to code it correctly or remove it entirely select(-timeStart)
         gender = recode_factor(gender,
                                M = "Male",
                                F = "Female"),
         q5 = max(q5)-q5+1, # better to not redefine variables over itself
         average1 = (q1+q2+q3+q4+q5+q6)/6, # or use: rowMeans(across(q1:q6))
         average2 = (q8+q9+q10)/3,
         # could create a new column for true false of before July
         beforeJuly = timeEnd>"2017-06-30 23:59:59")
```
...
# another way to formate correlation
str_remove(formatC(analysis$estimate, digits=2), "^0")
```
...
```{r}
week8_tbl %>%
  select(timeEnd, gender, average1, average2) %>% # could use beforeJuly instead of timeEnd
